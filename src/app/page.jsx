...
const DEMO_ITEMS = {
  studs: [
    { id: 'd1', name: 'Rose Gold Mini Studs', price: '₹499', description: 'Delicate rose gold plated studs with matte finish', customizable: true, imageUrl: 'https://firebasestorage.googleapis.com/v0/b/demo/o/stud1.jpg?alt=media' },
  ...
  earrings: [
    { id: 'd4', name: 'Jhumka Traditional', price: '₹899', description: 'Handcrafted brass jhumkas with antique finish', customizable: true, imageUrl: 'https://firebasestorage.googleapis.com/v0/b/demo/o/ear1.jpg?alt=media' },
              ...
              <div key={cat.id} className={`cat-card${open ? ' open' : ''}`}>
                <button className="cat-head" onClick={() => setExpanded(open ? null : cat.id)}>
                  <div>
                    <h3>{cat.label}</h3>
                    <p>{cat.desc}</p>
                    {settings.customTagline && <p className="custom-tagline">{settings.customTagline}</p>}
                  </div>
                    ...
                    {loading ? (
                      ...
                      <div className="item-grid">
                        {(items[cat.id] || []).map(item => {
                          const msg = "Hi, I'm interested in \"" + item.name + "\" from " + cat.label + (item.customizable ? " (customization)" : "")
                          const waLink = "https://wa.me/91" + wa + "?text=" + encodeURIComponent(msg)
                          return (
                          <div key={item.id} className="item-card">
                            ...
                            <div className="item-info">
                              ...
                              {item.customizable && <span className="custom-badge">Customization Available</span>}
